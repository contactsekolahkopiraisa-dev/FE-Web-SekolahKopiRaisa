...
import { fetchProductById, updateProduct } from "@/app/utils/product";
...
export default function EditProductPage() {
  ...
  // State for partner name (untuk preview saja)
  const [partnerName, setPartnerName] = useState("Mitra Anda");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await updateProduct(Number(productId), formData);
      ...
      try {
        ...
        setProduct({
          ...
          stock: productData?.inventory?.stock
            ? String(productData.inventory.stock)
            : "",
        });

        setImagePreview(productData?.image || null);

        // Set partner name untuk preview (jika ada)
        if (productData?.partner?.name) {
          setPartnerName(productData.partner.name);
        }
      } catch (error) {
    ...
    <div className="mx-auto bg-tertiary p-6 rounded-xl shadow-lg">
      ...
      {isLoadingProduct ? (
        <div className="flex justify-center items-center py-20">
          ...
          <span className="ml-3">Memuat data produk...</span>
        </div>
      ) : (
